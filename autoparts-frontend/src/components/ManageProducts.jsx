...
export default function ManageProducts() {
  ...
  const [draft, setDraft] = useState({
    id: null, name: '', part: '', brand: '', price: '', stock: '',
    image_path: '', imageFile: null,
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      setDraft({ ...draft, imageFile: file, image_path: previewUrl });
    }
  };

  async function handleSubmit() {
    setError('');
    const { id, name, part, brand, price, stock, imageFile, description, specifications } = draft;
    ...
    if (!brand || !brand.trim()) {
      return setError('Vui lòng chọn thương hiệu xe.');
    }
    ...
    if (!id && !imageFile && !draft.image_path) {
    ...
    } else if (draft.image_path && !id) {
      if (!draft.image_path.startsWith('http')) return setError('Vui lòng chọn ảnh sản phẩm.');
      formData.append('image_path', draft.image_path);
    }
    ...
    <div className="grid lg:grid-cols-2 gap-6">
      ...
      <Card className="h-fit">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              >
                ...
                <option value="noithat">Nội thất (noithat)</option>
                <option value="ngoaithat">Ngoại thất (ngoaithat)</option>
                <option value="thietbi">Thiết bị (thietbi)</option>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Thương hiệu xe</label>
              <select
                className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-sm"
                value={draft.brand}
          ...
          {/* Description & Specs */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Mô tả sản phẩm (Hỗ trợ HTML)</label>
            <textarea
              className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm h-32 font-mono"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Thông số kỹ thuật (JSON hoặc Text)</label>
            <textarea
              className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm h-24 font-mono"
          ...
          <div>
            ...
            <label className={`flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg cursor-pointer hover:bg-gray-50 transition ${draft.image_path ? 'border-blue-300 bg-blue-50' : 'border-gray-300'}`}>
          ...
          {draft.image_path && (
            <div className="relative w-full h-48 bg-gray-100 rounded-lg overflow-hidden border border-gray-200">
              <img src={draft.image_path} alt="Preview" className="w-full h-full object-contain"
          ...
          {list.map(p => (
            <div key={p.id} className="flex items-center gap-3 border border-gray-100 rounded-xl p-3 hover:bg-blue-50 hover:border-blue-200 transition bg-white group shadow-sm">
              <div className="w-16 h-16 rounded-lg border border-gray-200 overflow-hidden flex-shrink-0 bg-gray-50">
                <img src={p.image_path} className="w-full h-full object-cover" alt={p.name}
              ...
              <div className="flex-1 min-w-0 cursor-pointer" onClick={() => startEdit(p)}>
                ...
                <div className="text-xs text-gray-500 mt-0.5 flex items-center gap-1">
                  <span className="bg-gray-100 px-1.5 py-0.5 rounded text-gray-600 border border-gray-200">
                    {PART_LABELS[p.part] || (p.part === 'noithat' ? 'Nội thất' : p.part === 'ngoaithat' ? 'Ngoại thất' : p.part === 'thietbi' ? 'Thiết bị' : p.part)}
                  </span>
                  <span className="bg-blue-100 px-1.5 py-0.5 rounded text-blue-600 border border-blue-200">
                    {p.brand}
                  </span>
                </div>
